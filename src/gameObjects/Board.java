...
public class Board extends GameObject implements Cloneable, Iterable<Point> {
	...
	public boolean flanked(Point attacked, Point attacker) {
		...
		if (centre != null) {
			...
			if (left(attacked) != null)
				left = pieceAt(left(attacked));
			if (right(attacked) != null)
				right = pieceAt(right(attacked));
			if (above(attacked) != null)
				above = pieceAt(above(attacked));
			if (below(attacked) != null)
				below = pieceAt(below(attacked));

			// enemy left and right
			if (centre.isEnemy(left) && centre.isEnemy(right))
				if (left.getPoint().equals(attacker) || right.getPoint().equals(attacker))
					involvesAttacker = true;
			// enemy above and below
			if (centre.isEnemy(above) && centre.isEnemy(below))
				if (above.getPoint().equals(attacker) || below.getPoint().equals(attacker))
					involvesAttacker = true;
			// on left edge with two enemies around
			if (attacked.x == 0 && centre.isEnemy(right) && ((centre.isEnemy(above) || centre.isEnemy(below))))
				if (above.getPoint().equals(attacker) || below.getPoint().equals(attacker) || right.getPoint().equals(attacker))
					involvesAttacker = true;
			// on right edge with two enemies around
			if (attacked.x == XDIM - 1 && centre.isEnemy(left) && ((centre.isEnemy(above) || centre.isEnemy(below))))
				if (above.getPoint().equals(attacker) || below.getPoint().equals(attacker) || left.getPoint().equals(attacker))
					involvesAttacker = true;
			// on top edge with two enemies around
			if (attacked.y == 0 && centre.isEnemy(below) && ((centre.isEnemy(left) || centre.isEnemy(right))))
				if (below.getPoint().equals(attacker) || left.getPoint().equals(attacker) || right.getPoint().equals(attacker))
					involvesAttacker = true;
			// on bottom edge with two enemies around
			if (attacked.y == YDIM - 1 && centre.isEnemy(above) && ((centre.isEnemy(left) || centre.isEnemy(right))))
				if (above.getPoint().equals(attacker) || left.getPoint().equals(attacker) || right.getPoint().equals(attacker))
					involvesAttacker = true;
		}
		return involvesAttacker;
	}
